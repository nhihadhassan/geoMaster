...
export function PremiumControls() {
  ...
  if (isCompact) {
    return (
      <motion.button
        ...
        className="absolute bottom-24 left-3 z-20 rounded-full border border-white/12 bg-zinc-950/52 px-4 py-2 text-sm font-semibold text-white/70 shadow-lg shadow-black/25 backdrop-blur-xl transition hover:bg-zinc-950/64 hover:text-white sm:bottom-28 sm:left-5"
      >
  ...
  return (
    <motion.aside
      ...
      animate={{ opacity: 1, x: 0, scale: 1 }}
      transition={{ type: "spring", stiffness: 260, damping: 28 }}
      className="absolute inset-x-3 bottom-24 z-20 max-h-[min(64dvh,30rem)] overflow-y-auto rounded-3xl border border-white/12 bg-zinc-950/56 p-3 text-white shadow-xl shadow-black/30 backdrop-blur-xl sm:inset-x-auto sm:bottom-28 sm:left-5 sm:w-[min(20rem,calc(100vw-2.5rem))] sm:max-h-[calc(100vh-12rem)]"
    >
      <div>
              ...
              <button
                ...
                className={`rounded-2xl border px-3 py-2 text-left text-sm font-semibold transition ${
                  isSelected || selectedSpecialRegion === region.id
                    ...
                    : region.enabled
                      ? "border-white/10 bg-white/[0.055] text-white/58 hover:bg-white/10 hover:text-white"
